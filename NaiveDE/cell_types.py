...
def logistic_model(data, cell_types, sparsity=0.2, fraction=0.5, Solver='liblinear'): #auesro mod (Apparently, LogisticRegression in scikit learn changed the default solver to one not accepting l1 penalty since this code was written. Changed default solver to liblinear.)
...
def plot_roc(y_prob, y_test, lr):
    ...
    auc = pd.DataFrame(data=None) #auesro mod (Create dataframe to store AUC values)
    for i, cell_type in enumerate(lr.classes_):
        fpr, tpr, _ = metrics.roc_curve(y_test == cell_type, y_prob[:, i])
        AUC = metrics.roc_auc_score(y_test == cell_type, y_prob[:, i]) #auesro mod (calculate AUC for each group)
        auc[cell_type] = [AUC] #auesro mod (add AUC value to each group)
        plt.plot(fpr, tpr, c=ct_colors[i], lw=2)

    plt.plot([0, 1], [0, 1], color='k', ls=':')
    plt.xlabel('FPR')
    plt.ylabel('TPR')
    return auc #auesro mod (return auc values)
